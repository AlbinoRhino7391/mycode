#!/usr/bin/env python3
"""Eric Phelps edit to complete challenges
...
def movielookup_by_year(mykey, searchstring, year):
    """searches OMDB API by string and year"""
    api = f"{OMDBURL}apikey={mykey}&s={searchstring}&y={year}"
    resp = requests.get(api)
    return resp.json()

def movielookup_by_type_and_year(mykey, searchstring, movietype, year):
    api = f"{OMDBURL}apikey={mykey}&s={searchstring}&type={movietype}&y={year}"
    resp = requests.get(api)
    return resp.json()
    ...
    while True:
        ...
        while answer == "":
            ...
            ** Returned data will be written into the local database **
            ...
            5) Show what movies you have already Searched.
            99) Exit""")
        ...
        if answer in ["1", "2"]:
            ...
            if answer == "1":
                ...
                resp = movielookup_by_year(mykey, searchstring, year)
            elif answer == "4":
                movietype = input("Enter the movie type (e.g., movie, series, episode): ")
                year = input("Enter the year of release: ")
                resp = movielookup_by_type_and_year(mykey, searchstring, movietype, year)
            elif answer == "5":
                print_local_db()
            if resp:
                # display the results
